...
**narrativediff** plugs into your stack through [`cognis-connect`](https://github.com/cognis-digital/cognis-connect),
...
Once `narrativediff` emits JSON findings, pipe them straight to a destination — `--dry-run`
previews the exact request without sending:

```bash
narrativediff ... --format json | cognis-connect emit --to stix   # STIX 2.1 bundle
narrativediff ... --format json | cognis-connect emit --to slack --url $URL --token $TOK   # Slack
narrativediff ... --format json | cognis-connect emit --to webhook --url $URL --token $TOK   # generic webhook
```
...
works whatever `narrativediff` outputs:

```python
from cognis_connect import normalize, stix
findings = [normalize(rec, source="narrativediff") for rec in records]   # records = your JSON output
