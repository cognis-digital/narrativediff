"""NARRATIVEDIFF — JSON rule engine."""
...
TOOL_VERSION = "0.1.0"
RULES = [{'id': 'ND-FRAME-001', 'severity': 'medium', 'weight': 2.0, 'title': 'FRAMING_DIVERGENCE', 'when': {'framing_divergence': '^(0\\.[6-9]|1\\.0)'}, 'remediation': 'Compare side-by-side; consider for editorial newsletter.'}, {'id': 'ND-OMIT-001', 'severity': 'medium', 'weight': 2.0, 'title': 'FACT_OMISSION', 'when': {'omission_score': '^(0\\.[7-9]|1\\.0)'}, 'remediation': 'Surface omission to readers.'}]
        ...
        for rule in RULES:
            if _match(item, rule):
                result.add(Finding(
                    ...
                    location=str(target),
                    remediation=rule.get("remediation",""), category="media-bias",
                    metadata=item,
                ))
