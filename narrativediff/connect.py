"""Native cognis-connect emit for narrativediff — forward findings to any platform.

Maps narrativediff's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    narrativediff ... --format json | narrativediff-emit --to stix
    narrativediff-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    ...
    try:
        out = dict(rec)
        out['title'] = rec.get('event', '')
        out['severity'] = rec.get('bias_magnitude', 'info')
        out['type'] = 'narrative_diff'
        out['description'] = rec.get('event', '')
        out['tags'] = [rec.get('outlet', '')]
        out['ipv4'] = ''
        out['domain'] = ''
        out['url'] = ''
        out['sha256'] = ''
